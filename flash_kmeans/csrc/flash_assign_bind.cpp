...
std::vector<torch::Tensor> flash_assign_cuda(torch::Tensor points, torch::Tensor centroids) {
    ...
    cudaError_t err = launch_flash_assign_complete_256x128x32(
    ...
    TORCH_CHECK(err == cudaSuccess, "launch_flash_assign_complete_256x128x32 failed: ", cudaGetErrorString(err));

    return {output_ids, output_dists, point_norms, centroid_norms};
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("flash_assign_cuda", &flash_assign_cuda, "Flash assign CUDA");
}
