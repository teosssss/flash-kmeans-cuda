...
def _load_extension():
    ...
    return load(
        name="flash_assign_cuda_ext_bench",
        sources=[
...
def _bench_case(ext, num_points: int, num_centroids: int, dim: int, warmup: int, iters: int, seed: int) -> None:
    ...
    print(
        f"BENCH shape=(M={num_points}, N={num_centroids}, K={dim}) "
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark flash_assign CUDA vs Triton")
    parser.add_argument(
        "--cases",
        nargs="*",
        default=["1024,512,128", "4096,1024,128", "4096,1024,256"],
